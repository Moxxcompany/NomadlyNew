      ...
      <div className="dashboard-container">
        <header className="header" data-testid="header">
          <div className="logo-section">
            ...
            <h1 className="logo-text">NomadlyBot</h1>
          </div>
          <div className="status-badge" data-testid="status-badge" data-status={status}>
            <span className="status-dot"></span>
            <span className="status-label">
              {status === 'loading' ? 'Checking...' : status === 'online' ? 'System Online' : status === 'degraded' ? 'Degraded' : 'Offline'}
            </span>
          </div>
        </header>
        ...
        <main className="main-content" data-testid="main-content">
          <section className="hero-section">
            ...
            <p className="hero-subtitle">
              URL Shortening &middot; Domain Sales &middot; Phone Leads &middot; Crypto Payments &middot; Web Hosting
            </p>
          </section>
          ...
          <div className="stats-grid" data-testid="stats-grid">
            <StatCard 
              title="Bot Status"
              value={status === 'online' ? 'Running' : status === 'loading' ? '...' : 'Stopped'}
              detail="Telegram Bot Engine"
              accent="emerald"
            />
            <StatCard 
              title="Database"
              value={botHealth?.db === 'connected' ? 'Connected' : status === 'loading' ? '...' : 'N/A'}
              detail="MongoDB Instance"
              accent="sky"
            />
            <StatCard 
              title="REST APIs"
              value={status === 'online' ? 'Active' : status === 'loading' ? '...' : 'Inactive'}
              detail="Express Server"
              accent="violet"
            />
            <StatCard 
              title="Services"
              value="5+"
              detail="Integrated Modules"
              accent="amber"
            />
          </div>

          <div className="features-grid" data-testid="features-grid">
            <FeatureCard icon="link" title="URL Shortener" desc="Custom domain shortening with analytics" />
...
function FeatureCard({ icon, title, desc }) {
  ...
  return (
    <div className="feature-card" data-testid={`feature-card-${icon}`}>
